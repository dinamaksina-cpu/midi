...
(function(){
  ...
  function initAnimations(){
    ...
    if(q('#heroOrb') && !reduce){gsap.to('#heroOrb',{rotate:360,duration:42,repeat:-1,ease:'none'});}
    initHeroExperience();
  ...
  function initHeroExperience(){
    const hero=q('.hero'), shell=q('#heroOrbMotion'), title=q('#heroTitle'), role=q('#heroRole');
    ...
    if(window.ScrollTrigger){
      gsap.timeline({scrollTrigger:{trigger:hero,start:'top top',end:'bottom top',scrub:.7}})
        ...
        .to('#heroOrbMotion',{y:-48,scale:.91,opacity:.42,ease:'none'},0)
